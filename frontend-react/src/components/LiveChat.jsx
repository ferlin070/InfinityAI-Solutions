...
import { 
  MessageSquare, User, Bot, Send, UserCheck, RefreshCw 
} from 'lucide-react';
import { 
...
export default function LiveChat({ t }) {
  ...
  const activeConvName = activeConv?.contacts?.name || activeConv?.contacts?.phone || 'WhatsApp Client';

  return (
    <div className="glass-card overflow-hidden grid grid-cols-1 md:grid-cols-3 h-[600px]">
      {/* Sidebar List */}
        ...
        <div className="flex-1 overflow-y-auto divide-y divide-border/30">
          {conversations.length > 0 ? (
            conversations.map((c) => {
              ...
              const convName = c.contacts?.name || c.contacts?.phone || 'Unknown Client';
              return (
                <div 
                  key={c.id} 
                ...
                >
                  ...
                  <div className="flex items-center justify-between text-[10px]">
                    ...
                    <div className="flex items-center space-x-1.5">
                      {c.mode === 'ai' ? (
                        <span className="px-1.5 py-0.5 rounded bg-primary-subtle text-primary font-medium flex items-center">
                          <Bot className="w-2.5 h-2.5 mr-0.5" /> AI
                        </span>
                      ) : (
                        <span className="px-1.5 py-0.5 rounded bg-accent-gold/10 text-accent-gold font-medium flex items-center">
                          <User className="w-2.5 h-2.5 mr-0.5" /> Staff
                        </span>
                      )}
                      <span className={`w-1.5 h-1.5 rounded-full ${c.status === 'open' ? 'bg-accent-success' : 'bg-text-faint'}`} />
                    </div>
                  </div>
                </div>
              );
            ...
            <div className="h-full flex items-center justify-center text-xs text-text-muted p-6 text-center">
              Tiada perbualan aktif. Apabila pelanggan menghantar mesej WhatsApp, ia akan muncul di sini.
            </div>
          )}
        </div>
      </div>
      ...
      <div className="md:col-span-2 flex flex-col h-full">
        {activeConv ? (
          <>
            <div className="p-4 border-b border-border flex items-center justify-between">
              <div>
                <h4 className="font-semibold text-sm">{activeConvName}</h4>
              ...
              {activeConv.mode === 'ai' && (
                <button 
                  ...
                  className="flex items-center text-[10px] font-semibold text-accent-gold bg-accent-gold/10 border border-accent-gold/20 hover:bg-accent-gold/20 px-3 py-1.5 rounded transition-all"
                >
                    ...
                    >
                      <div className={`max-w-[75%] rounded-lg p-3 text-xs space-y-1 ${
                        ...
                        <div className="flex items-center space-x-1.5 text-[10px] text-text-muted mb-0.5">
                          {m.sender === 'ai' && <Bot className="w-3 h-3 text-primary" />}
                          {m.sender === 'staff' && <User className="w-3 h-3 text-primary" />}
          ...
          <div className="h-full flex flex-col items-center justify-center text-text-muted p-6 space-y-2">
