...
import { Save, Play, Sparkles, MessageCircle, Bot } from 'lucide-react';
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="glass-card p-5 space-y-4 h-fit">
        ...
        <p className="text-xs text-text-muted">Pilih ejen untuk mengubah suai prompt sistem atau konfigurasi model.</p>
          ...
          {AGENT_KEYS.map((a) => (
            <button
              ...
              className={`w-full text-left p-3 rounded-lg text-xs transition-all flex flex-col space-y-1 ${
                selectedAgent === a.key 
                  ? 'bg-primary-subtle border border-primary/30 font-semibold' 
      ...
      <div className="glass-card lg:col-span-2 p-5 space-y-4">
        <div className="flex items-center justify-between border-b border-border pb-3">
          <div>
            ...
            <p className="text-xs text-text-muted">Edit prompt sistem, model, dan provider untuk ejen ini.</p>
          </div>
        ...
        {message && (
          <div className={`p-3 rounded text-xs ${
            message.type === 'success' ? 'bg-accent-success/10 text-accent-success' : 'bg-accent-danger/10 text-accent-danger'
          }`}>
            {message.text}
          </div>
        )}
        ...
        <div className="space-y-1.5">
          ...
          <textarea
            ...
            className="w-full bg-background border border-border focus:border-primary/50 outline-none p-3 rounded text-xs font-mono leading-relaxed"
          />
        </div>
        ...
        <div className="border-t border-border pt-4 mt-6 space-y-3">
          <h4 className="text-xs font-bold text-text-muted uppercase tracking-wider flex items-center">
            <Sparkles className="w-3.5 h-3.5 mr-1.5 text-accent-gold" />
          ...
          <div className="flex space-x-2">
            ...
            <button 
              ...
              className="flex items-center text-xs bg-primary/25 border border-primary/40 hover:bg-primary/45 text-text px-4 py-2.5 rounded-lg font-semibold transition-colors disabled:opacity-50"
            >
          ...
          {simOutput && (
            <div className="p-3 bg-surface-raised border border-border rounded text-xs space-y-2">
