...
Today this is a **monolith**: the FastAPI backend serves the API *and* the frontend as one process. `backend/src/api/routes.py` reads and returns `frontend/src/index.html` directly, and mounts `frontend/src/{icons,assets}` as static file routes. **`frontend/src/` is build output, not source** — it's generated by `npm run build` in `frontend-react/` (a React 19 + Vite SPA; `vite.config.js` points `outDir` at `../frontend/src`), and is gitignored. Something has to run that build before `frontend/src/` exists — either you, locally, or the Docker image (see below). Historically `frontend/src/` was hand-authored vanilla HTML/CSS/JS and was committed straight to git; if you're reading an older doc (or an AI session) that still describes that, it's stale — see `frontend/README.md`.

This has two consequences for this guide:

1. **The simple path (recommended right now):** deploy the whole repo as one service to Railway or Render using the **root** `Dockerfile` (see §3). It builds `frontend-react/` in a Node stage, then serves the result + the API from one FastAPI process at the same URL. No Vercel involved.
...
**Two Dockerfiles, not interchangeable:**
- **Root `Dockerfile`** — multi-stage: Node builds `frontend-react/`, then Python serves backend + built dashboard together. This is the monolith. Build context must be the **repository root** (it `COPY`s from both `backend/` and `frontend-react/`).
- **`backend/Dockerfile`** — Python only, build context `backend/`. It **cannot** reach `frontend-react/` (outside its build context — Docker forbids that categorically, it's not a config toggle), so it's **API-only**: no dashboard baked in. An earlier version of this file tried `COPY ../frontend /app/frontend` anyway, which would fail every real `docker build` from a `backend/` context — if you're deploying via "Root Directory: backend" anywhere, that's why it broke. Use it only for something that genuinely doesn't need the dashboard (e.g. the `worker` service in `docker-compose.yml`).
...
2. **Root Directory:** leave as the repo root (`.`), **not** `backend` — Render needs the root to reach both `backend/` and `frontend-react/` for the monolith build (see §0's Dockerfile note). **Environment:** Docker. Render will find the root `Dockerfile` automatically.
...
7. If you only want the API (no dashboard) on Render, e.g. as a second internal service, that's the one case where Root Directory `backend` + `backend/Dockerfile` is correct — just know it serves no frontend at all.
...
3. **Frontend API base URL** (`frontend-react/src/api.js`) currently calls relative paths (`fetch('/api/...')`). Deployed separately on Vercel, that resolves against Vercel's own domain, not the backend. Needs a configurable base URL (e.g. a Vite env var — `import.meta.env.VITE_API_BASE_URL` — read at build time) pointing at the Railway/Render URL from §3.
...
Once those land, Vercel setup itself is a standard Vite project: New Project → import repo → **Root Directory:** `frontend-react` → Framework Preset: Vite → Build Command: `npm run build` → Output Directory: `dist` (note: **not** `frontend/src` — that outDir is only for the monolith Docker path; a standalone Vercel deploy should build to Vite's default `dist/` instead, which means overriding `vite.config.js`'s `build.outDir` per-environment or maintaining a separate Vercel-specific config) → deploy.
