...
def test_build_crewai_agent_omits_image_tool_without_collector():
    ...
    # DANISH may have other (static / browser) tools, but without an
    # artifact_collector the Image Generation tool must not be present —
    # there'd be nowhere for the generated image to go.
    assert all(t.name != "Image Generation" for t in agent.tools)
